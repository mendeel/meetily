...
pub(crate) fn create_transcript_segments(transcripts: &[(String, f64, f64)]) -> Vec<TranscriptSegment> {
    create_transcript_segments_with_speakers(
        &transcripts
            .iter()
            .map(|(text, start_ms, end_ms)| (text.clone(), *start_ms, *end_ms, None))
            .collect::<Vec<_>>(),
    )
}

/// Create transcript segments including optional speaker labels.
/// Each tuple is (text, start_ms, end_ms, speaker).
