...
import { DEFAULT_NEMOTRON_MODEL } from '../constants/modelDefaults';
...
export function NemotronModelManager({
  ...
  useEffect(() => {
    ...
    const initializeModels = async () => {
      try {
        setLoading(true);
        await NemotronAPI.init();
        const modelList = await NemotronAPI.getAvailableModels();
        setModels(modelList);

        setInitialized(true);
      } catch (err) {
        console.error('Failed to initialize Nemotron:', err);
  ...
  // Set up event listeners for download progress and model loading
  useEffect(() => {
    let unlistenProgress: (() => void) | null = null;
    let unlistenComplete: (() => void) | null = null;
    let unlistenError: (() => void) | null = null;
    let unlistenLoadingStarted: (() => void) | null = null;
    let unlistenLoadingCompleted: (() => void) | null = null;
    let unlistenLoadingFailed: (() => void) | null = null;

    const setupListeners = async () => {
      console.log('[NemotronModelManager] Setting up event listeners...');
      ...
      unlistenProgress = await listen<{ modelName: string; progress: number }>(
        ...
        (event) => {
          ...
          if (shouldUpdate) {
            console.log(`[NemotronModelManager] Progress update for ${modelName}: ${progress}%`);
      ...
      // Model loading events (match Parakeet naming pattern)
      unlistenLoadingStarted = await listen<{ modelName: string }>(
        'nemotron-model-loading-started',
        (event) => {
          const { modelName } = event.payload;
          console.log(`[NemotronModelManager] Loading started: ${modelName}`);
        }
      );

      unlistenLoadingCompleted = await listen<{ modelName: string }>(
        'nemotron-model-loading-completed',
        (event) => {
          const { modelName } = event.payload;
          const displayName = getModelDisplayName(modelName);
          console.log(`[NemotronModelManager] Loading completed: ${modelName}`);
      ...
      unlistenLoadingFailed = await listen<{ modelName: string; error: string }>(
        'nemotron-model-loading-failed',
        (event) => {
          const { modelName, error } = event.payload;
          const displayName = getModelDisplayName(modelName);
          console.error(`[NemotronModelManager] Loading failed: ${modelName}`, error);
          toast.error(`Failed to load ${displayName}`, {
            description: error,
            duration: 5000
          });
        }
    ...
    return () => {
      console.log('[NemotronModelManager] Cleaning up event listeners...');
      if (unlistenProgress) unlistenProgress();
      if (unlistenComplete) unlistenComplete();
      if (unlistenError) unlistenError();
      if (unlistenLoadingStarted) unlistenLoadingStarted();
      if (unlistenLoadingCompleted) unlistenLoadingCompleted();
      if (unlistenLoadingFailed) unlistenLoadingFailed();
    };
  ...
  const cancelDownload = async (modelName: string) => {
    ...
    try {
      await NemotronAPI.cancelDownload(modelName);
  ...
  const downloadModel = async (modelName: string) => {
    ...
    try {
      ...
      await NemotronAPI.downloadModel(modelName);
    } catch (err) {
  ...
  const deleteModel = async (modelName: string) => {
    ...
    try {
      await NemotronAPI.deleteCorruptedModel(modelName);
